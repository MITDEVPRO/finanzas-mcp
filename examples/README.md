...
Cuatro casos de una misma empresa ficticia — una **distribuidora chilena mediana** (cifras en millones de CLP) — resueltos con las tools de finanzas-mcp. Cada ejemplo trae la pregunta como se la harías a tu cliente MCP (Claude Desktop, Claude Code u otro), los parámetros exactos y la **salida real** de la tool, sin editar.
...
Los cuatro se encadenan: el WACC alimenta el DCF, y el Z-Score más el punto de equilibrio le ponen contexto de riesgo a esa valoración.
...
```bash
uv run --with pytest pytest   # 31 tests sobre las mismas funciones
```
